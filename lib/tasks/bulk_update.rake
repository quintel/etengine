...
namespace :bulk_update do
  ...
  task :update_scenarios => :environment do
    ...
    counter = 0
    scenarios_with_bad_buildings = Array.new
    Scenario.order('id').find_each(:batch_size => 100) do |s|
      ...
      if sum>0.0 
        if !(sum).round(1).between?(99.9, 100.1)
          ...
          scenarios_with_bad_buildings << s.id
        end
      end 
      ...
      # Cost curves for the micro CHP should be initialized at the correct
      # cost (depends on start-year of scenario)
      if s.end_year == 2013
        inputs[595] = -20.0
      elsif s.end_year == 2014
  ...
  puts "These scenarios have bad buildings space heating sectors (shares do not sum to 100%): "
  puts scenarios_with_bad_buildings
  end
end
