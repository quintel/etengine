...
describe Api::V3::InputsController do
 ...
 describe 'GET /api/v3/scenarios/:scenario_id/inputs/:id' do
  ...
  describe 'GET /api/v3/scenarios/:scenario_id/inputs.csv' do
    let(:csv) { JSON.parse(get(:index, params: { scenario_id: scenario.id, format: :json }).body) }
    ...
    it 'should contain each input' do
      expect(csv).to have_key(static_input.key)
      expect(csv).to have_key(gql_input.key)
    end

    it 'does not have a "code" attribute for each input' do
      expect(csv[static_input.key]).not_to have_key('code')
      expect(csv[gql_input.key]).not_to    have_key('code')
    end

    it 'should have a "min" attribute for each input' do
      expect(csv[static_input.key]).to include('min' => 5)
      expect(csv[gql_input.key]).to    include('min' => 4)
    end

    it 'should have a "max" attribute for each input' do
      expect(csv[static_input.key]).to include('max' => 15)
      expect(csv[gql_input.key]).to    include('max' => 16)
    end

    it 'should have a "default" attribute for each input' do
      expect(csv[static_input.key]).to include('default' => 10)
      expect(csv[gql_input.key]).to    include('default' =>  8)
    end
    ...
    context '"disabled" attribute' do
      ...
      it 'should be true when an input is disabled' do
        expect(csv[static_input.key]).to include('disabled' => true)
      end

      it 'should be false when an input is not disabled' do
        expect(csv[gql_input.key]).to include('disabled' => false)
      end
    end
    ...
    context '"label" attribute' do
      ...
      it 'should be present when an input has a label' do
        expect(csv[static_input.key]).to \
          include('label' => { 'value' => 32.0, 'suffix' => 'g'})
      end

      it 'should not be present when an input is not disabled' do
        expect(csv[gql_input.key]).not_to have_key('label')
      end
    end # "label" attribute
    ...
    context '"user" attribute' do
      ...
      it 'should be present when an input has a user value' do
        expect(csv[gql_input.key]).to include('user' => 42.0)
      end

      it 'should not be present when an input does not have a user value' do
        expect(csv[static_input.key]).not_to have_key('user')
      end
    end # "user" attribute
    ...
    context 'with a scaled scenario' do
      ...
      it 'scales static input values' do
        expect(csv[static_input.key]).to include(
          'min'     =>  5 / divisor,
          'max'     => 15 / divisor,
          'default' => 10 / divisor
        )
      ...
      it 'does not scale GQL-based input values' do
        # GQL inputs are not scaled, since they use the local graph to compute
        # their values.
        expect(csv[gql_input.key]).to include(
          'min'     =>  4,
          'max'     => 16,
          'default' =>  8
        )
    ...
    context 'with an enum input' do
      ...
      it 'omits the min value' do
        expect(csv[gql_input.key]).not_to have_key('min')
      end

      it 'omits the max value' do
        expect(csv[gql_input.key]).not_to have_key('max')
      end

      it 'omits the step value' do
        expect(csv[gql_input.key]).not_to have_key('step')
      end
