...
RSpec.describe Qernel::Plugins::Hydrogen::Calculator do
  ...
  context 'with constant 5 demand, 5 supply' do
    ...
    it 'has no residual demand' do
      expect(calculator.residual_demand).to eq([0.0] * 10)
    end

    it 'has no cumulative residual demand' do
      expect(calculator.cumulative_residual_demand).to eq([0.0] * 10)
    end
  ...
  context 'with constant 5 demand, variable (1-41) supply' do
    ...
    it 'has residual demand of -4 and 36' do
      expect(calculator.residual_demand).to eq([
        -4, -4, -4, -4, -4, -4, -4, 36, -4, -4
      ])
    end

    it 'has cumulative residual demand' do
      expect(calculator.cumulative_residual_demand).to eq([
  ...
  context 'with variable demand, constant 5 supply' do
    ...
    it 'has residual demand' do
      expect(calculator.residual_demand.map { |v| v.round(1) }).to eq([
        -3.3, -4.3, -1.3, 1.7, 3.7, 3.7, 2.7, 0.7, -1.3, -2.3
      ])
    end

    it 'has cumulative residual demand' do
      expect(calculator.cumulative_residual_demand.map { |v| v.round(1) })
        .to eq([
