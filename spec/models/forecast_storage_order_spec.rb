...
describe ForecastStorageOrder do
  ...
  describe '#order' do
    ...
    context 'when the order contains one of two valid values' do
      let(:order) { [Etsource::Config.forecast_storage_order.first] }
    ...
    context 'when the order contains all valid values' do
      let(:order) { Etsource::Config.forecast_storage_order }
    ...
    context 'when the order repeats a valid value' do
      let(:order) do
        [
          Etsource::Config.forecast_storage_order.first,
          Etsource::Config.forecast_storage_order.first
        ]
      end
