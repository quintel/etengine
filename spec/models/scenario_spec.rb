...
describe Scenario do
  ...
  describe '#user_values' do
    context ':user_values = YAMLized object with string keys and string values (when coming from db)' do
      before { @scenario = described_class.new(user_values: { foo: :bar }) }

      it 'unyaml,s overwrite and return user_values' do
    ...
    context ':user_values = nil' do
      before { @scenario = described_class.new(user_values: nil) }
    ...
    context ':user_values = obj' do
      before { @scenario = described_class.new(user_values: {}) }
  ...
  describe 'with a preset scenario' do
    ...
    context 'with a custom interconnector 1 electricity price curve' do
      ...
      let!(:scenario) { create(:scenario) }
      let(:curve_data) do
        File.read(Rails.root.join('spec/fixtures/files/price_curve.csv')).lines.map(&:to_f)
      end
      let(:preset_curve) do
        create(:user_curve,
          scenario: preset,
  ...
  context 'with two scenarios using the same curve values' do
    ...
    it 'deleting one curve does not delete the other' do
      expect do
        curve_one.destroy
      end.to change(UserCurve, :count).by(-1)
  ...
  describe '#metadata' do
    ...
    context 'when setting metadata' do
      ...
      it 'denies objects larger than 64Kb' do
        scenario.metadata = (0..15_000).index_by(&:to_s)
