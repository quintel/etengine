module Api
  module V3
    ...
    module ConverterPresenterData
      ...
      # add these
      ELECTRICITY_PRODUCTION_ATTRIBUTES_AND_METHODS  = SHARED_ATTRIBUTES_AND_METHODS.deep_merge({
        :technical => {
      ...
      # add these
      HEAT_PRODUCTION_ATTRIBUTES_AND_METHODS  = SHARED_ATTRIBUTES_AND_METHODS.deep_merge({
        :technical => {
      ...
      # add these
      HEAT_PUMP_ATTRIBUTES_AND_METHODS  =  SHARED_ATTRIBUTES_AND_METHODS.deep_merge({
        :technical => {
          :heat_output_capacity =>
            {label: 'Nominal heat capacity', unit:'MW'},
      ...
      # add these
      CHP_ATTRIBUTES_AND_METHODS  = SHARED_ATTRIBUTES_AND_METHODS.deep_merge({
        :technical => {
          :electricity_output_capacity =>
            {label: 'Nominal electrical capacity', unit: 'MW'},
          :heat_output_capacity =>
      ...
      def attributes_and_methods_to_show
        out = {}
        out = out.deep_merge HEAT_PRODUCTION_ATTRIBUTES_AND_METHODS        if @converter.groups.include?(:cost_traditional_heat)
        out = out.deep_merge ELECTRICITY_PRODUCTION_ATTRIBUTES_AND_METHODS if @converter.groups.include?(:cost_electricity_production)
        out = out.deep_merge HEAT_PUMP_ATTRIBUTES_AND_METHODS              if @converter.groups.include?(:cost_heat_pumps)
        out = out.deep_merge CHP_ATTRIBUTES_AND_METHODS                    if @converter.groups.include?(:cost_chps)
