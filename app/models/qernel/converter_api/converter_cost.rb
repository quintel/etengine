...
# Calculates the variable costs for the converter per MWh input, including variable CCS costs.
  ...
  def total_costs
    # puts "total_costs: #{values_for_method(:total_costs)} : #{inspect} "
  ...
  def number_of_units
    if val = dataset_get(:number_of_units)
      return val
    end
  ...
  def production_based_on_number_of_units 	##only if still needed!
    return nil if required_attributes_contain_nil?(:production_based_on_number_of_units)
  ...
  def typical_electricity_production
    return nil if required_attributes_contain_nil?(:typical_electricity_production)
  ...
  def installed_production_capacity_in_mw_electricity ###instead of electricity_production_in_mw
    return nil if required_attributes_contain_nil?(:installed_production_capacity_in_mw_electricity)
