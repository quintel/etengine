...
#
#
module UpdateInterface::UpdatingConverter
  ...
  def after_calculation_updates(graph)
    cmd = UpdateInterface::AfterCalculationUpdate.new(graph)
    cmd.execute
  end
    ...
    graph.time_curves.andand.each do |converter_id, curve|
      if converter = graph.converter(converter_id)
        curve.each do |attr_name, curve|
          ...
          if UpdateInterface::SlotConversionCommand.responsible?(attr_name)
            cmds << UpdateInterface::SlotConversionCommand.new(converter, attr_name, curve[Current.scenario.end_year])
          else
            cmds << UpdateInterface::AttributeCommand.new(converter.proxy, attr_name, curve[Current.scenario.end_year], 'value')
          end
        end
      end
    end
  ...
  def update_area_data(graph, area_data_updates)
    ...
    area_data_updates.andand.each do |id, updates|
      updates.each do |key, value|
        cmds << UpdateInterface::CommandFactory.create(graph, area, key, value)
      end
    end
    execute_commands(cmds)
  end
  ...
  def update_carriers(graph, carrier_updates)
    ...
    carrier_updates.andand.each do |ids, updates|
      ids.split('_AND_').reject(&:blank?).each do |id|
        ...
        updates.each do |key,value|
          cmds << UpdateInterface::CommandFactory.create(graph, future, key, value)
        end
      end
    end
    execute_commands(cmds)
      ...
      select(select_query.to_s, graph).each do |converter|
        ...
        updates.each do |key, value|
          ...
          if "replacement_of_households_rate" == key
            ...
            cmds << UpdateInterface::CommandFactory.create(graph, proxy, key, value)
            if reverse_converter = future_converter(converter.full_key.to_s.gsub("heat_", "cold_"))
              cmds << UpdateInterface::CommandFactory.create(graph, reverse_converter, "cooling_output_link_share", value)
            end
          elsif c = UpdateInterface::CommandFactory.create(graph, proxy, key, value)
            cmds << c
          end
  ...
  def update_replacement_of_households_rate(graph, proxy, value)
    ...
    future_demand_old_houses_cmd = UpdateInterface::AttributeCommand.new(proxy, :preset_demand, value, :decrease_rate)
    ...
    cmds << UpdateInterface::AttributeCommand.new(graph.area, :number_of_existing_households, number_of_existing_households, :value)
    cmds << future_demand_old_houses_cmd
    cmds << UpdateInterface::AttributeCommand.new(new_house_converter, :preset_demand, new_houses_future_demand_value, :value)
